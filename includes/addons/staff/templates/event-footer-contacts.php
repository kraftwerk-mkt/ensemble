<?php
/**
 * Event Footer Contacts Template
 * 
 * Displays assigned contacts in the event footer
 * 
 * @package Ensemble
...
<section class="es-event-contacts">
    <h3 class="es-event-contacts__title">
    ...
    <div class="es-event-contacts__list">
        <?php foreach ($staff as $person) : ?>
            <div class="es-event-contact" itemscope itemtype="https://schema.org/Person">
                <?php if ($person['featured_image']) : ?>
                    <div class="es-event-contact__image">
                        <img src="<?php echo esc_url($person['featured_image']); ?>" 
                             alt="<?php echo esc_attr($person['name']); ?>"
                             itemprop="image">
                    </div>
                <?php else : ?>
                    <div class="es-event-contact__image es-event-contact__image--placeholder">
                ...
                <div class="es-event-contact__info">
                    <h4 class="es-event-contact__name" itemprop="name">
                    ...
                    <div class="es-event-contact__links">
                        <?php if (!empty($person['email'])) : ?>
                            <a href="mailto:<?php echo esc_attr($person['email']); ?>" 
                               class="es-event-contact__email"
                               itemprop="email"
                        ...
                        ?>
                            <a href="tel:<?php echo esc_attr(preg_replace('/[^0-9+]/', '', $phone['number'])); ?>" 
                               ...
                               title="<?php echo esc_attr($phone_label); ?>">
                                ...
                                <span class="es-event-contact__phone-type">(<?php echo esc_html($phone_label); ?>)</span>
                            </a>
                        <?php 
                                endif;
                            endforeach;
                        elseif (!empty($person['phone'])) : 
                            // Fallback for single phone (backward compatibility)
                        ?>
                            <a href="tel:<?php echo esc_attr(preg_replace('/[^0-9+]/', '', $person['phone'])); ?>" 
                               class="es-event-contact__phone"
                               itemprop="telephone"
                               title="<?php esc_attr_e('Call', 'ensemble'); ?>">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="16" height="16">
                                    <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/>
                                </svg>
                    ...
                    if ($has_social) : 
                    ?>
                        <div class="es-event-contact__social">
                            <?php if (!empty($person['website'])) : ?>
                                <a href="<?php echo esc_url($person['website']); ?>" 
                                   class="es-event-contact__social-link"
                                   target="_blank"
