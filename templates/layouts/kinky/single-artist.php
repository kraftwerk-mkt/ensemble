<?php
/**
 * Single Artist Template - KINKY LAYOUT
 * 
...
<div class="es-kinky-single es-kinky-artist-single">
    ...
    <!-- HERO SECTION -->
    <header class="es-kinky-hero es-kinky-hero-artist">
        ...
        <div class="es-kinky-hero-content">
            ...
            <span class="es-kinky-hero-genre"><?php echo esc_html($artist['genre']); ?></span>
            <?php endif; ?>
            
            <h1 class="es-kinky-hero-title"><?php the_title(); ?></h1>
            
            <?php if (!empty($artist['origin'])): ?>
            <span class="es-kinky-hero-origin"><?php echo esc_html($artist['origin']); ?></span>
            <?php endif; ?>
            
            <!-- Social Links in Hero -->
            <?php if ($has_social): ?>
            <div class="es-kinky-hero-social">
                <?php if (!empty($social_links['spotify'])): ?>
                <a href="<?php echo esc_url($social_links['spotify']); ?>" target="_blank" rel="noopener" title="Spotify" class="es-kinky-social-link">
                ...
                <a href="<?php echo esc_url($social_links['soundcloud']); ?>" target="_blank" rel="noopener" title="SoundCloud" class="es-kinky-social-link">
                ...
                <a href="<?php echo esc_url($social_links['youtube']); ?>" target="_blank" rel="noopener" title="YouTube" class="es-kinky-social-link">
                    <svg viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                </a>
                <?php endif; ?>
                <?php if (!empty($social_links['instagram'])): ?>
                <a href="<?php echo esc_url($social_links['instagram']); ?>" target="_blank" rel="noopener" title="Instagram" class="es-kinky-social-link">
                ...
                <a href="<?php echo esc_url($social_links['facebook']); ?>" target="_blank" rel="noopener" title="Facebook" class="es-kinky-social-link">
                    <svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
                <?php endif; ?>
                <?php if (!empty($social_links['mixcloud'])): ?>
                <a href="<?php echo esc_url($social_links['mixcloud']); ?>" target="_blank" rel="noopener" title="Mixcloud" class="es-kinky-social-link">
                ...
                <a href="<?php echo esc_url($social_links['bandcamp']); ?>" target="_blank" rel="noopener" title="Bandcamp" class="es-kinky-social-link">
                ...
                <a href="<?php echo esc_url($social_links['website']); ?>" target="_blank" rel="noopener" title="Website" class="es-kinky-social-link">
    ...
    <div class="es-kinky-container">
        <div class="es-kinky-content-wrapper">
            
            <!-- Artist Type / Role -->
            <?php if (!empty($artist['type'])): ?>
            <div class="es-kinky-artist-type">
                <span class="es-kinky-type-label"><?php echo esc_html($artist['type']); ?></span>
            </div>
            <?php endif; ?>
            
            <!-- Bio -->
            <?php if (get_the_content()): ?>
            <section class="es-kinky-section es-kinky-bio">
                <?php if (function_exists('ensemble_before_artist_content')) ensemble_before_artist_content($artist_id); ?>
                <h2 class="es-kinky-section-title"><?php _e('Biografie', 'ensemble'); ?></h2>
            ...
            <section class="es-kinky-section es-kinky-references">
                <blockquote class="es-kinky-quote">
                    <span class="es-kinky-quote-mark">"</span>
            ...
            <section class="es-kinky-section es-kinky-events">
                <h2 class="es-kinky-section-title"><?php _e('Kommende Auftritte', 'ensemble'); ?></h2>
                <div class="es-kinky-divider"></div>
                
                <?php if (function_exists('ensemble_artist_events')) ensemble_artist_events($artist_id); ?>
                    ...
                    <a href="<?php echo esc_url($evt['permalink']); ?>" class="es-kinky-event-row">
                        ...
                        <div class="es-kinky-event-info">
                            ...
                            <?php if (!empty($evt['location']) && is_array($evt['location']) && !empty($evt['location']['name'])): ?>
                                <span class="es-kinky-event-location"><?php echo esc_html($evt['location']['name']); ?></span>
                            <?php endif; ?>
                        </div>
                ...
                <?php if (function_exists('ensemble_after_artist_events')) ensemble_after_artist_events($artist_id, $upcoming_events); ?>
            </section>
...
<!-- Kinky Single Artist Styles -->
<style>
/* Artist Single Specific Styles */
.es-kinky-artist-single {
...
.es-kinky-hero-genre {
    ...
    margin-bottom: 16px;
    padding: 8px 16px;
    border: 1px solid var(--kinky-primary, #cc2222);
}

.es-kinky-hero-origin {
    display: block;
    font-family: var(--kinky-font-body, 'Lato', sans-serif);
    font-size: 14px;
    font-weight: 300;
...
.es-kinky-social-link:hover {
    border-color: var(--kinky-primary, #cc2222);
    color: var(--kinky-primary, #cc2222);
    box-shadow: var(--kinky-glow);
}

.es-kinky-social-link svg {
    width: 18px;
    height: 18px;
}

/* Artist Type Badge */
.es-kinky-artist-type {
    text-align: center;
    margin-bottom: 48px;
}
...
.es-kinky-section:last-child {
    border-bottom: none;
    padding-bottom: 0;
}

.es-kinky-section-title {
...
.es-kinky-divider {
    width: 60px;
    height: 2px;
    background: linear-gradient(90deg, var(--kinky-primary, #cc2222), var(--kinky-secondary, #ff6633));
    margin: 0 auto 32px;
}
...
.es-kinky-quote {
    ...
    padding: 32px 48px;
    margin: 0;
    border-left: 3px solid var(--kinky-primary, #cc2222);
...
.es-kinky-event-list {
    ...
    flex-direction: column;
    gap: 1px;
    background: var(--kinky-card-border, #1a1a22);
}
...
.es-kinky-event-row:hover {
    background: rgba(204, 34, 34, 0.1);
}

.es-kinky-event-row:hover .es-kinky-event-name {
    color: var(--kinky-secondary, #ff6633);
}
...
.es-kinky-event-arrow {
    color: var(--kinky-text-muted, #8a8a9a);
    transition: all 0.3s ease;
}

.es-kinky-event-row:hover .es-kinky-event-arrow {
    color: var(--kinky-primary, #cc2222);
    transform: translateX(4px);
}

.es-kinky-event-arrow svg {
    width: 20px;
    height: 20px;
}
