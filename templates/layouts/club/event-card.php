<?php
/**
 * Template: Club Event Card
 * Dark nightclub style with date badge overlay
 * Image is edge-to-edge, padding only in content area
 * 
...
<article class="ensemble-event-card es-club-card <?php echo ($is_special && $show_status) ? 'es-status-' . esc_attr($status) : ''; ?>" <?php echo $card_data_attributes; ?>>
    <a href="<?php echo esc_url($permalink); ?>" class="es-club-link">
        ...
        <div class="es-club-image">
            ...
            <div class="es-club-status es-club-status-<?php echo esc_attr($status); ?>">
        ...
        <div class="es-club-content">
            ...
            <span class="es-club-genre"><?php echo esc_html($category_name); ?></span>
            <?php endif; ?>
            
            <?php if ($show_title): ?>
            <h3 class="es-club-title"><?php echo esc_html($title); ?></h3>
            <?php endif; ?>
            
            <?php if (($start_time && $show_time) || ($location && $show_location)): ?>
            <div class="es-club-meta">
                <?php if ($start_time && $show_time): ?>
                <span class="es-club-time"><?php echo esc_html($start_time); ?></span>
                <?php endif; ?>
                
                <?php if ($location && $show_location): ?>
                <span class="es-club-location"><?php echo esc_html($location); ?></span>
                <?php endif; ?>
            </div>
            <?php endif; ?>
            
            <?php if ($price && !$is_special && $show_price): ?>
            <span class="es-club-price"><?php echo esc_html($price); ?></span>
            <?php endif; ?>
