...
<article class="ensemble-event-card es-lovepop-card es-lovepop-card--overlay <?php echo ($is_special && $show_status) ? 'es-status-' . esc_attr($status) : ''; ?>" <?php echo $card_data_attributes; ?>>
    <a href="<?php echo esc_url($permalink); ?>" class="es-lovepop-card-link">
        ...
        <div class="es-lovepop-card-bg">
            ...
            <?php else: ?>
                <div class="es-lovepop-placeholder-bg"></div>
            <?php endif; ?>
        ...
        <span class="es-lovepop-card-status"><?php echo esc_html(function_exists('ensemble_get_status_label') ? ensemble_get_status_label($status, true) : ucfirst($status)); ?></span>
        <?php endif; ?>
        
        <!-- Content Overlay -->
        <div class="es-lovepop-card-overlay">
            
            <!-- Date (visible by default, hidden on hover) -->
            <?php if ($timestamp && $show_date): ?>
            <div class="es-lovepop-card-date">
                <span class="es-lovepop-card-day"><?php echo date_i18n('j', $timestamp); ?></span>
                <span class="es-lovepop-card-month"><?php echo date_i18n('M', $timestamp); ?></span>
            </div>
            <?php endif; ?>
            
            <!-- Artists (hidden by default, visible on hover) -->
            <?php if (!empty($artists)): ?>
            <div class="es-lovepop-card-artists">
                <?php 
            ...
            <!-- Title -->
            <?php if ($show_title): ?>
            <h3 class="es-lovepop-card-title"><?php echo esc_html($title); ?></h3>
            <?php endif; ?>
